...
## Scope
- Applies to `/crates/krusty-core` and its direct contents.
- If a deeper directory has its own `AGENTS.md`, that file takes precedence for its subtree.

## Purpose
Core runtime crate providing AI clients, tools, storage, planning, and protocols.
...
## Structure Map
### Subdirectories
- `src/`: Core crate source modules exported by krusty-core. See `crates/krusty-core/src/AGENTS.md` for local detail.
