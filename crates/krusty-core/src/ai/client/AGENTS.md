# AGENTS Guide: /crates/krusty-core/src/ai/client

## Scope
- Applies to `/crates/krusty-core/src/ai/client` and its direct contents.
- If a deeper directory has its own `AGENTS.md`, that file takes precedence for its subtree.

## Purpose
AI client request/streaming orchestration and provider-specific adapters.
...
- `core.rs`: Rust source module implementing core behavior.
- `mod.rs`: Module root that wires child modules and shared exports.
- `request_builder.rs`: Rust source module implementing request builder behavior.
- `simple.rs`: Rust source module implementing simple behavior.
- `streaming.rs`: Rust source module implementing streaming behavior.
- `thinking.rs`: Rust source module implementing thinking behavior.
