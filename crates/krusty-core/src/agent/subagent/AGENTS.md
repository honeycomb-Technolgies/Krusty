# AGENTS Guide: /crates/krusty-core/src/agent/subagent

## Scope
- Applies to `/crates/krusty-core/src/agent/subagent` and its direct contents.
- If a deeper directory has its own `AGENTS.md`, that file takes precedence for its subtree.

## Purpose
Sub-agent execution pipeline and tool envelopes.
...
### Files
- `execution.rs`: Rust source module implementing execution behavior.
