# AGENTS Guide: /crates/krusty-core/src/agent

## Scope
- Applies to `/crates/krusty-core/src/agent` and its direct contents.
- If a deeper directory has its own `AGENTS.md`, that file takes precedence for its subtree.

## Purpose
Agent orchestration, hooks, events, context building, and lifecycle state.
...
## Structure Map
### Subdirectories
- `subagent/`: Sub-agent execution pipeline and tool envelopes. See `crates/krusty-core/src/agent/subagent/AGENTS.md` for local detail.

### Files
- `build_context.rs`: Rust source module implementing build context behavior.
- `cache.rs`: Rust source module implementing cache behavior.
- `cancellation.rs`: Rust source module implementing cancellation behavior.
- `constants.rs`: Rust source module implementing constants behavior.
- `event_bus.rs`: Rust source module implementing event bus behavior.
- `events.rs`: Rust source module implementing events behavior.
- `hooks.rs`: Rust source module implementing hooks behavior.
- `mod.rs`: Module root that wires child modules and shared exports.
- `pinch_context.rs`: Rust source module implementing pinch context behavior.
- `state.rs`: Rust source module implementing state behavior.
- `summarizer.rs`: Rust source module implementing summarizer behavior.
- `user_hooks.rs`: Rust source module implementing user hooks behavior.
