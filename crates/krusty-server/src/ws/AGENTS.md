# AGENTS Guide: /crates/krusty-server/src/ws

## Scope
- Applies to `/crates/krusty-server/src/ws` and its direct contents.
- If a deeper directory has its own `AGENTS.md`, that file takes precedence for its subtree.

## Purpose
WebSocket transport handlers including terminal streams.
...
- `terminal.rs`: Rust source module implementing terminal behavior.
