# AGENTS Guide: /crates/krusty-cli/src/tui/markdown

## Scope
- Applies to `/crates/krusty-cli/src/tui/markdown` and its direct contents.
- If a deeper directory has its own `AGENTS.md`, that file takes precedence for its subtree.

## Purpose
Markdown parser, cache, and renderer for terminal message content.
...
- `elements.rs`: Rust source module implementing elements behavior.
- `hyperlinks.rs`: Rust source module implementing hyperlinks behavior.
- `inline.rs`: Rust source module implementing inline behavior.
- `links.rs`: Rust source module implementing links behavior.
- `mod.rs`: Module root that wires child modules and shared exports.
- `parser.rs`: Rust source module implementing parser behavior.
