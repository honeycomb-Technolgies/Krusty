# AGENTS Guide: /crates/krusty-cli/src/tui/polling

## Scope
- Applies to `/crates/krusty-cli/src/tui/polling` and its direct contents.
- If a deeper directory has its own `AGENTS.md`, that file takes precedence for its subtree.

## Purpose
Polling loops that sync external process/auth/tool states.
...
- `mcp.rs`: Rust source module implementing mcp behavior.
- `mod.rs`: Module root that wires child modules and shared exports.
- `oauth.rs`: Rust source module implementing oauth behavior.
