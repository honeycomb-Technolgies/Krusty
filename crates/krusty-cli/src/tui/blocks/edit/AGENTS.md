# AGENTS Guide: /crates/krusty-cli/src/tui/blocks/edit

## Scope
- Applies to `/crates/krusty-cli/src/tui/blocks/edit` and its direct contents.
- If a deeper directory has its own `AGENTS.md`, that file takes precedence for its subtree.

## Purpose
Specialized edit block state and rendering internals.
...
### Files
- `block.rs`: TUI stream block module responsible for block rendering and interaction.
- `mod.rs`: Module root that wires child modules and shared exports.
- `render.rs`: TUI stream block module responsible for render rendering and interaction.
