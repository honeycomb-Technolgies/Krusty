# AGENTS Guide: /crates/krusty-cli/src/tui/plugins

## Scope
- Applies to `/crates/krusty-cli/src/tui/plugins` and its direct contents.
- If a deeper directory has its own `AGENTS.md`, that file takes precedence for its subtree.

## Purpose
Terminal plugin integrations (graphics, gamepad, retro components).
...
### Files
- `gamepad.rs`: Rust source module implementing gamepad behavior.
- `kitty_graphics.rs`: Rust source module implementing kitty graphics behavior.
- `libretro.rs`: Rust source module implementing libretro behavior.
- `mod.rs`: Module root that wires child modules and shared exports.
- `retroarch.rs`: Rust source module implementing retroarch behavior.
