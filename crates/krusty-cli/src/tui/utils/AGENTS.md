# AGENTS Guide: /crates/krusty-cli/src/tui/utils

## Scope
- Applies to `/crates/krusty-cli/src/tui/utils` and its direct contents.
- If a deeper directory has its own `AGENTS.md`, that file takes precedence for its subtree.

## Purpose
Shared terminal utility helpers for text, syntax, channels, and worktrees.
...
### Files
- `channels.rs`: Rust source module implementing channels behavior.
- `languages.rs`: Rust source module implementing languages behavior.
- `mod.rs`: Module root that wires child modules and shared exports.
- `syntax.rs`: Rust source module implementing syntax behavior.
- `text.rs`: Rust source module implementing text behavior.
- `title.rs`: Rust source module implementing title behavior.
- `worktree.rs`: Rust source module implementing worktree behavior.
