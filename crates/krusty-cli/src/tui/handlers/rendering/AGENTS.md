# AGENTS Guide: /crates/krusty-cli/src/tui/handlers/rendering

## Scope
- Applies to `/crates/krusty-cli/src/tui/handlers/rendering` and its direct contents.
- If a deeper directory has its own `AGENTS.md`, that file takes precedence for its subtree.

## Purpose
Render pipeline orchestration and viewport calculations.
...
## Structure Map
### Subdirectories
- `messages/`: Message render helpers and selection behavior. See `crates/krusty-cli/src/tui/handlers/rendering/messages/AGENTS.md` for local detail.

### Files
- `mod.rs`: Module root that wires child modules and shared exports.
- `scroll_calc.rs`: Event-handling module implementing scroll calc behavior.
- `views.rs`: Event-handling module implementing views behavior.
