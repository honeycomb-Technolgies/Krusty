# AGENTS Guide: /apps/pwa/app/static

## Scope
- Applies to `/apps/pwa/app/static` and its direct contents.
- If a deeper directory has its own `AGENTS.md`, that file takes precedence for its subtree.

## Purpose
Static PWA assets (icons, manifest, favicon).
...
### Files
- `favicon.png`: Static image asset used by favicon UI surfaces.
- `icon-192.png`: Static image asset used by icon 192 UI surfaces.
- `icon-512.png`: Static image asset used by icon 512 UI surfaces.
- `icon.svg`: Static image asset used by icon UI surfaces.
- `manifest.json`: PWA manifest describing install metadata and app icons.
